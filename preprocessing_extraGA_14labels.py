...
if __name__ == "__main__":
    ...
    # CSF             1 L   8 R
    # GrayMatter      2 L   9 R
    # WhiteMatter     3 L   10 R
    # Ventricles      4 L   11 R
    # Cerebellum      5 L   12 R
    # DeepGreyMatter  6 L   13 R 
    # BrainStem       7 L   14 R
        
    training_folder = "/homedtic/malenya/nnUNet/DB_SpatialConstraints/Training" 
    testing_folder = "/homedtic/malenya/nnUNet/DB_SpatialConstraints/Testing"
    output_folder = "/homedtic/malenya/nnUNet/FeTA_SpatialConstraints_CL_Mat/nnUNet_raw_data_base/nnUNet_raw_data/Task001_Brain/" # empty
    ...
    train_folder = "/homedtic/malenya/nnUNet/FeTA_SpatialConstraints_CL_Mat/nnUNet_raw_data_base/nnUNet_raw_data/Task001_Brain/imagesTr"
    test_folder = "/homedtic/malenya/nnUNet/FeTA_SpatialConstraints_CL_Mat/nnUNet_raw_data_base/nnUNet_raw_data/Task001_Brain/imagesTs"
    
    
    for i in range(60):
        image = nib.load(os.path.join(train_folder,"{:0>3}_0000.nii.gz".format(i+1)))
        ga_np = np.ones(image.shape) * gas[i]
        ga_image = image.__class__(ga_np,image.affine, image.header)
        nib.save(ga_image, os.path.join(train_folder,"{:0>3}_0001.nii.gz".format(i+1)))
    for i in range(60,80):
        image = nib.load(os.path.join(test_folder,"{:0>3}_0000.nii.gz".format(i+1)))
        ga_np = np.ones(image.shape) * gas[i]
        ga_image = image.__class__(ga_np,image.affine, image.header)
        nib.save(ga_image, os.path.join(test_folder,"{:0>3}_0001.nii.gz".format(i+1)))
    
    
    json_dict = OrderedDict()
    json_dict['name'] = "FeTA_Dataset"
    json_dict['description'] = "FeTA_SpatialConstraints"
